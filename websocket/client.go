...
import (
	...
	log2 "github.com/showbaba/query-bridge/bridge-core/log"
...
func StreamHandler(ctx context.Context, mongoClient *mongo.Client) fiber.Handler {
	return websocket.New(func(c *websocket.Conn) {
		...
		for {
			...
			if err != nil {
				log.Error("error validating auth token: %v", err)
				return
			}
			...
			if err != nil {
				log.Info("MongoDB query error:", err)
				return
			}
			...
			for cursor.Next(ctx) {
				...
				if err := cursor.Decode(&logObj); err != nil {
					log.Error("Cursor decoding error:", err)
					return
				}
